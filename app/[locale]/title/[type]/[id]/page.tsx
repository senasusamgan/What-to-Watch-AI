...
export default async function TitleDetail({
  ...
  const item = await getDetails(type, id, locale);
  ...
  const duration =
    type === "movie" ? item.runtime : item.episode_run_time?.[0];
  ...
  const trailer =
    ...
    item.videos?.results?.find((video: any) => video.site === "YouTube");
  ...
  const turkeyWatchProviders = item.watchProviders?.results?.TR;
  ...
  return (
    <main className="min-h-screen bg-black text-white">
            ...
            <div className="max-w-4xl pb-4">
              ...
              {item.genres?.length > 0 && (
                <div className="mt-5 flex flex-wrap gap-2">
                  {item.genres.map((genre: { id: number; name: string }) => (
                    <span
                      key={genre.id}
      ...
      <div className="mx-auto max-w-7xl space-y-20 px-6 pb-20">
        <div id="watch-providers" className="scroll-mt-8">
          <WatchProviders providers={turkeyWatchProviders} />
        </div>
        ...
        {trailer && (
          <section id="trailer" className="scroll-mt-8">
            <div className="mb-6 flex items-center gap-3">
              ...
              <h2 className="text-3xl font-bold">{t("trailer")}</h2>
            </div>
        ...
        {recommendations.length > 0 && (
          <section>
            <div className="mb-6 flex items-center gap-3">
              <div className="h-8 w-1 rounded-full bg-red-600" />
              <h2 className="text-3xl font-bold">{t("similar")}</h2>
            </div>
